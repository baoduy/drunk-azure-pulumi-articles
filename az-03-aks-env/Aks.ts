...
/**
 * This AKS we will using Linux with vmSize is `Standard_B2ms` and enable auto scale min: 1 node and max 3 nodes.
 * */
export default (
  name: string,
  {
  ...
  const aks = new azure.containerservice.ManagedCluster(
    aksName,
    {
      ...
      networkProfile: {
        ...
        outboundType: azure.containerservice.OutboundType.UserDefinedRouting,
